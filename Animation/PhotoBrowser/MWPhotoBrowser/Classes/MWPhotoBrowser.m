...
//    _pageControll = [[UIPageControl alloc] initWithFrame:CGRectMake(0, kScreenHeight-40, kScreenWidth, 40)];
//    _pageControll.currentPageIndicatorTintColor = kSubjectMainColor;
//    _pageControll.pageIndicatorTintColor = [UIColor whiteColor];
//    _pageControll.numberOfPages = [self numberOfPhotos];
...
- (UIButton *)deleteBtn {
    if (!_deleteBtn) {
        _deleteBtn = [[UIButton alloc]initWithFrame:CGRectMake([[UIScreen mainScreen] bounds].size.width - 44, 30, 24, 24)];
        [_deleteBtn setImage:[UIImage imageNamed:@"yhshape"] forState:UIControlStateNormal];
...
- (void)delegateBtnClick {
    if (self.deleteBlock) {
        self.deleteBlock(_currentPageIndex);
    }
}
...
//    _pageControll.currentPage = _currentPageIndex;
	// Buttons
...
- (void)toggleControls:(UIImageView *)object {
    //1. 设置代理
    self.animatedTransition = nil;
    self.navigationController.delegate = self.animatedTransition;
    
    //2. 传入必要的参数
    [self.animatedTransition setOriginView:object];
    [self.animatedTransition setOriginImage:object.image];
    [self.animatedTransition setBackColor:[UIColor whiteColor]];
    CGRect frame = self.getFrame(((MWZoomingScrollView *)object.superview).index);
    [self.animatedTransition setDestinationFrame:frame];
    //3.push跳转
...
//    [AppDelegateHelper showMulAlertView:[AppDelegate topViewController] style:UIAlertControllerStyleActionSheet title:nil message:@"图片保存" cancelButtonTitle:@"取消" otherButtonTitles:@[@"保存到本地"] cancelHandler:^{
//
//    } otherButtonHandles:@[^{
//        [self savePhoto];
//    }]];
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    [self showProgressHUDCompleteMessage: error ? NSLocalizedString(@"请重试", @"Informing the user a process has failed") : NSLocalizedString(@"图片已保存", @"Informing the user an item has been saved")];
    [self hideControlsAfterDelay]; // Continue as normal...
}
